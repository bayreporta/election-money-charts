	// ------- GLOBAL ------- //
		...
		var raceTotal = [5,5]; //manually enter the number of candidates for each race, in order you placed in list above
		var raceColumns = [2,2]; // manually enter the column where this race data starts on the Summary sheet of GSheet
...
// ------- GET AND POPULATE CANDIDATE INFO ------- //
	...
	function drawTopDonors(){
		...
		for (i=0 ; i < raceTotal[race]; i++){
			...
			data[i][2] = mainDB[12][ii];
			data[i][3] = "$" + commaSeparateNumber(parseInt(mainDB[11][ii]));
			data[i][4] = mainDB[14][ii];
			data[i][5] = "$" + commaSeparateNumber(parseInt(mainDB[13][ii]));
			data[i][6] = mainDB[16][ii];
			data[i][7] = "$" + commaSeparateNumber(parseInt(mainDB[15][ii]));
			data[i][8] = mainDB[18][ii];
			data[i][9] = "$" + commaSeparateNumber(parseInt(mainDB[17][ii]));
			data[i][10] = mainDB[20][ii];
			data[i][11] = "$" + commaSeparateNumber(parseInt(mainDB[19][ii]));
			ii = ii + 1;
		}
	...
	function drawSpent(){
		...
		var ii = raceColumns[race];
		for (i=0 ; i < raceTotal[race] ;  i++){
			data[i] = parseInt(candidateDB[18][ii]);
			commas[i] = commaSeparateNumber(data[i]);
			ii = ii + 1;
		}
	...
	function drawGeography(){
		...
		if (stacked === 0){
			...
			for (i=0 ; i < raceTotal[race] ;  i++){
				...
				commas[i][3] = commaSeparateNumber(data[i][3]);
				ii = ii + 1;
			}
